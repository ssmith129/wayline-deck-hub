...
1. **Slide 19 is missing from the markup.** Section comments run 18 → 20. Runtime confirms **25 main `<section>` slides while numbering runs to 26.** Resolve before the interview loop — either restore a slide or renumber so the chrome counter is honest. *(Unresolved as of this writing.)*
...
1. **T3 decision** on the slide-7 MP4 — confirm it plays/loops/rests-on-result, or fall back to its poster PNG.
1. **T2** — annotations on 6 and 17. *(Done.)*
1. **T4** — backup charts for 4/11/12. *(Done — B1–B3.)*
1. **Slide 9 two-state** and **slide 16 diagram** — only after 1–4. Both `[CUTTABLE]`.
...
1. Slide-9 two-state “fix” half.
1. Slide-13 Figma half (slide survives on the code panel alone).
1. Slide-16 distribution diagram (text version is defensible).
1. Backup B1 (verbal defense of 32% is nearly as good).
